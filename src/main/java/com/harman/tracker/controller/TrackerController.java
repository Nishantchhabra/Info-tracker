...
public class TrackerController {
	...
	public ResponseEntity<EmployeeInfo> getInfoById(@PathVariable(value="id") Long harmanId){
		...
		EmployeeInfo employee=trackerRepository.findById(harmanId).orElseThrow(()-> new ResourceNotFoundException(harmanId));
	...
	@PutMapping("/info/{id}")
	public ResponseEntity<EmployeeInfo> updateinfo(@PathVariable(value="id") Long harmanId ,@Valid @RequestBody EmployeeInfo employeeInfo){
		EmployeeInfo employee=trackerRepository.findById(harmanId).orElseThrow(()-> new ResourceNotFoundException(harmanId));
